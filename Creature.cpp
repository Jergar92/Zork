...
void Creature::Look()const{//LOOK CURRENT ROOM AND HIS ITEMS
...
void Creature::Look(Vector<MyString> &strings){
	int direction = -1;
	direction = App->getDirection(strings);//dat get the right direction
	if (direction == -1){
...
void Creature::Open(Vector<MyString> &strings){
	int direction = -1;
	direction = App->getDirection(strings);//dat get the right direction
	if (direction == -1){
		printf("wrong operation\n");
...
void Creature::Close(Vector<MyString> &strings){
	int direction = -1;
	direction = App->getDirection(strings);//dat get the right direction
	if (direction == -1){
		printf("wrong operation\n");
...
void Creature::Take(Vector<MyString> &strings){//TAKE OBJECTS
...
void Creature::Drop(Vector<MyString> &strings){
...
void Creature::Equip(Vector<MyString> &strings){
	if (!this->list.empty()){
		List<Entity*>::Node* item = this->list.first_data;
		for (; item != nullptr; item = item->next){
...
void Creature::UnEquip(Vector<MyString> &strings){
	if (!this->list.empty()){
		List<Entity*>::Node* item = this->list.first_data;
		for (; item != nullptr; item = item->next){
			if (strings[1] == ((Item*)item->data)->name && ((Item*)item->data)->isItem == BOOTS || ((Item*)item->data)->isItem == ARMOR || ((Item*)item->data)->isItem == WEAPON){
...
void Creature::Push(Vector<MyString> &strings){
	int direction = -1;
	List<Entity*>::Node* item = location->list.first_data;
...
void Creature::PutInto(Vector<MyString> &strings){
...
void Creature::GetFrom(Vector<MyString> &strings){//Get items froms trunk
	if (!location->list.empty()){
		List<Entity*>::Node* item = location->list.first_data;
		for (; item != nullptr; item = item->next){
